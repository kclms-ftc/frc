 ...
 * ----------------------
 * Like all subsystems, this class never calls HardwareMap.get().
...
public class Odometry {
    ...
    public double getVelocityY() {
        //Before: return pinpoint.getVelocity().getY(DistanceUnit.MM);
    ...
    public double getAngularVelocityDegPerSec() {
        // getHeadingVelocity() takes an AngleUnit, not a numeric argument.
        // AngleUnit.DEGREES is correct here; the Pinpoint driver handles values > 360 internally.
        return pinpoint.getHeadingVelocity(AngleUnit.DEGREES);
    }
    ...
    /**
     * stop() — no-op for Odometry; the Pinpoint device is passive.
     * Called by Robot.stopAll() to satisfy the subsystem contract.
     * The Pinpoint keeps tracking even at zero power — that's intentional.
     */
    public void stop() {
        // Pinpoint is a sensor, not an actuator — nothing to power down here.
    }
